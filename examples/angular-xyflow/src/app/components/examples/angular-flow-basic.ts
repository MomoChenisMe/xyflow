...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MiniMapComponent, BackgroundComponent, ControlsComponent],
  ...
  template: `
    ...
    >
      ...
      >
        <!-- 背景 -->
        <xy-background [variant]="backgroundVariantEnum.Dots" />
        ...
        >
          ...
          >
            ...
            <div
              ...
              style="pointer-events: none; transform-origin: 0 0;"
            >
            ...
            >
              ...
              <svg
                style="position: absolute; width: 100%; height: 100%; overflow: visible; pointer-events: none; z-index: 1001;"
              >
                <path
...
export class AngularFlowBasic implements AfterViewInit, OnDestroy {
  ...
  // 常量  
  public readonly backgroundVariantEnum = BackgroundVariant;
  ...
  public onNodeDragStart = output<{
    event: MouseEvent;
    node: Node;
    nodes: Node[];
  }>();
  public onNodeDrag = output<{ node: Node; nodes: Node[] }>();
  public onNodeDragStop = output<{
    event: MouseEvent;
    node: Node;
    nodes: Node[];
  }>();
  ...
  // 其他 MiniMap 需要的屬性
  public translateExtent = signal<[[number, number], [number, number]]>([
    [-Infinity, -Infinity],
    [Infinity, Infinity],
  ]);
  ...
  public minimapViewport = computed(() => {
    ...
    const boundingRect =
      visibleNodes.length > 0
        ? this.getBoundsOfRects(nodeBounds, viewBB)
        : viewBB;
  ...
  ngAfterViewInit() {
    ...
    setTimeout(() => {
      if (typeof window !== 'undefined') {
        ...
        console.log(
          '💡 Angular Flow 驗證工具已就緒！在控制台執行 verifyAngularFlow() 來測試修復效果'
        );
      }
    }, 1000);
  }
  ...
  private handleDragging(event: MouseEvent) {
    if (this.dragState.dragType === 'node' && this.dragState.dragNode) {
      // 計算新位置
      const deltaX =
        (event.clientX - this.dragState.startPos.x) / this.viewport().zoom;
      const deltaY =
        (event.clientY - this.dragState.startPos.y) / this.viewport().zoom;
  ...
  private stopConnection(event: MouseEvent) {
    ...
    console.log(
      'Element under mouse:',
      elementUnderMouse?.className,
      elementUnderMouse?.tagName
    );
    ...
    if (
      ...
      if (
        targetNodeId &&
        fromState.fromNode &&
        targetNodeId !== fromState.fromNode
      ) {
        console.log(
          `Creating connection: ${fromState.fromNode} -> ${targetNodeId}`
        );
  ...
  protected getEdgeZIndex(edge: Edge): number {
    ...
    const selectedZIndex =
      this.elevateEdgesOnSelect() && edge.selected ? 1000 : 0;
  ...
  protected getConnectionPath(): string {
    ...
    const sourceNode = this.internalNodes().find(
      (n) => n.id === state.fromNode
    );
  ...
  public deleteSelectedElements() {
    ...
    const remainingEdges = this.internalEdges().filter(
      (e) =>
        !e.selected &&
  ...
  private scaleViewportByFactor(factor: number) {
    ...
    // 獲取視口中心點
    const containerRect =
      this.flowContainer.nativeElement.getBoundingClientRect();
  ...
  public fitViewToNodes() {
    ...
    // 計算居中位置
    const x =
      containerWidth / 2 - (boundingBox.x + boundingBox.width / 2) * finalZoom;
    const y =
      containerHeight / 2 -
      (boundingBox.y + boundingBox.height / 2) * finalZoom;
  ...
  public toggleInteractive() {
    ...
    console.log(
      'Interactive state:',
      this.interactiveState() ? 'Unlocked' : 'Locked'
    );
  }
  ...
  private getBoundsOfRects(nodeBounds: any, viewBB: any) {
    ...
    const maxX = Math.max(
      ...
      viewBB.x + viewBB.width
    );
    const maxY = Math.max(
      nodeBounds.y + nodeBounds.height,
      viewBB.y + viewBB.height
    );
  ...
  protected getMinimapMaskPath(): string {
    ...
    const outerPath = `M${vbX - offset},${vbY - offset}h${
      vbWidth + offset * 2
    }v${vbHeight + offset * 2}h${-vbWidth - offset * 2}z`;
    ...
    if (!viewBB) {
      // fallback到轉換後的坐標
      const innerPath = `M${minimap.x},${minimap.y}h${minimap.width}v${
        minimap.height
      }h${-minimap.width}z`;
    ...
    const innerPath = `M${viewBB.x},${viewBB.y}h${viewBB.width}v${
      viewBB.height
    }h${-viewBB.width}z`;
