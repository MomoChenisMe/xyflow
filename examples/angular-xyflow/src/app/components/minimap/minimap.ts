...
@Component({
  ...
  template: `
    <div class="react-flow__minimap react-flow__panel bottom right" 
         [style]="panelStyle()"
         [class]="panelClass()"
...
export class MiniMap implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Filter function for hidden nodes
  ...
  // 計算 viewBox 和相關尺寸 - 與 React 版本完全相同
  private viewBoxCalculation = computed(() => {
  ...
  viewBox = computed(() => this.viewBoxCalculation().viewBox);
  
  // 計算遮罩路徑 - 與 React 版本完全相同
  maskPath = computed(() => {
    const { viewBB, x, y, width, height, offset } = this.viewBoxCalculation();
  ...
  panelStyle = computed(() => {
    ...
    const viewScale = this.viewBoxCalculation().viewScale;
  ...
  constructor() {
    ...
    effect(() => {
      this.viewScaleRef = this.viewBoxCalculation().viewScale;
    });
