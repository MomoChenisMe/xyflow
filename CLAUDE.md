...
**專業分工協作模式：**主 Agent 負責整體協調和實作，SubAgent 負責專業領域的研究規劃。

### 主 Agent 工作流程

#### 1. 建立專案上下文檔案
**檔案位置**：`.claude/context/PROJECT_CONTEXT_[任務類型]_[YYYYMMDD_HHMM].md`
...
#### 2. 委派任務給專業 SubAgent
**路徑導向委派**：根據檔案路徑自動確定委派目標
**任務類型導向委派**：根據任務性質選擇合適的專業 SubAgent
**委派內容**：傳遞上下文檔案路徑和具體任務需求
**協作模式**：可同時委派給多個 SubAgent 處理跨領域任務

#### 3. 讀取 SubAgent 計畫檔案
**等待所有 SubAgent 完成**：確認所有委派的 SubAgent 都已完成工作並產出計畫檔案
**計畫檔案位置**：`.claude/context/Angular_XYFlow_[任務類型]_[日期].md`

**優先順序分析**：當有多個計畫檔案時，分析技術依賴關係和實作順序
- 基礎架構優先（services → components → features）
- 依賴關係分析（被依賴者優先執行）
- 風險評估（高風險項目優先處理）

**內容驗證**：確認計畫完整性、技術可行性和架構一致性
**整合分析**：若有多個 SubAgent 參與，進行計畫整合和衝突解決
**決策確認**：基於專業計畫和優先順序制定最終實作策略

#### 4. 基於計畫執行實際實作
**依據優先順序執行**：按照步驟3分析的優先順序，依序執行各計畫檔案的實作
**依據專業計畫**：嚴格按照 SubAgent 提供的詳細實作計畫執行
**階段性實作**：將複雜實作分解為可管理的階段
**依賴關係管理**：確保前置依賴項目完成後再執行後續項目
**品質控制**：每個階段完成後進行代碼審查和測試
**進度追踪**：更新專案狀態和記錄實作過程

#### 5. 清理工作檔案
**完成後清理**：刪除上下文檔案和計畫檔案，保持專案整潔
...
**重要：** 根據檔案路徑和任務性質委派給相關的專業 SubAgent

#### 專業領域劃分

**react-to-angular-converter**
- **範圍**：`angular-xyflow/` 目錄下所有檔案
- **專精**：React Flow 到 Angular XYFlow 轉換、Angular 20+ signals 架構、Standalone components、現代 Angular 最佳實踐

#### 委派決策原則

**路徑導向委派**
- Angular XYFlow 專案層（`angular-xyflow/src/`）→ 委派給 react-to-angular-converter
- React Flow 範例轉換 → 委派給 react-to-angular-converter
- 跨框架整合分析 → 委派給 react-to-angular-converter

**任務類型導向委派**
- React 到 Angular 組件轉換 → react-to-angular-converter
- Angular Signals 狀態管理 → react-to-angular-converter
- XYFlow 功能實作 → react-to-angular-converter
- 性能優化和架構設計 → react-to-angular-converter

**協作模式**
- **單一委派**：明確屬於 Angular XYFlow 的任務
- **專業分析**：複雜 React 轉換需求的深度分析
- **架構整合**：SubAgent 完成後，在主專案層進行整合

**核心原則：**

✅ **Angular XYFlow 任務**：必須透過 Task tool 委派給 react-to-angular-converter
❌ **直接實作**：絕不直接處理 Angular XYFlow 相關的開發任務
🔄 **不確定時**：優先委派給 react-to-angular-converter 進行專業分析
